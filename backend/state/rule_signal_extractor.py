...
class RuleSignalExtractor:
    ...
    def extract(self, features) -> dict[UserState, float]:
        ...
        if getattr(features, "help_seeking_score", 0.0) >= 0.65:
            scores[UserState.STUCK] += 0.45
        if getattr(features, "answer_commitment_score", 0.0) <= 0.2 and features.retry_count >= 2:
        ...
        if getattr(features, "help_seeking_score", 0.0) >= 0.7 and features.message_length <= 20:
        ...
        if getattr(features, "answer_commitment_score", 0.0) >= 0.6:
